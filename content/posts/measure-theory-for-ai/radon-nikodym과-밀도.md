...
`\(\frac{dP}{dQ}<1\)`인 곳에서 `\(\log\frac{dP}{dQ}\)`는 음수다. 양수 부분의 적분과 음수 부분의 적분이 둘 다 무한대라면 `\(\infty-\infty\)`가 되어 KL이 정의되지 않을 것이다.

실제로는 그런 일이 일어나지 않는다. `\(r=\frac{dP}{dQ}\)`라 두고 11절의 측도 변환으로 적분을 `\(Q\)` 위로 옮기면
...
피적분함수에 나온 `\(\phi(t)=t\log t-t+1\)`은 `\(t\ge0\)`에서 항상 `\(0\)` 이상이다. `\(\phi'(t)=\log t\)`이므로 `\(\phi\)`는 `\(t<1\)`에서 감소하고 `\(t>1\)`에서 증가해, `\(t=1\)`에서 최솟값 `\(\phi(1)=0\)`을 갖는다. (`\(0\log0=0\)`으로 둔다.)
...
단, `\(P\ll Q\)`라고 해서 KL이 유한한 것은 아니다.

`\(P\)`를 표준 Cauchy 분포, `\(Q=N(0,1)\)`이라 하자. 두 밀도가 모든 점에서 양수이므로 `\(P\ll Q\)`다. 그런데


```math
\log\frac{p(x)}{q(x)} = \frac{x^2}{2}-\log(1+x^2)+\text{상수}
```


이고, Cauchy 분포는 `\(E_P[X^2]=\infty\)`다. (`\(\log(1+X^2)\)` 항의 기댓값은 유한하다.) 따라서
...
`\(P\ll Q\)`는 위의 적분 공식을 쓸 수 있게 해 주는 조건일 뿐, KL이 유한하다는 보장은 아니다.
